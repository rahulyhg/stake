  ...
  return {
	...
	createbook: function(bookname,date,horses,id) {
		...
		{	
			if(horses[i].name!="")
			{
				newhorses.push({id:i,name:horses[i].name,total:0});
			}
		}
		
		books.push({id:newbookid,date:date,name:bookname,horses:newhorses,bets:[]});	
	...
	insertonlyhorse: function(book,horsename,horseid) {
     	books[book].horses.push({id:horseid,name:horsename,total:0});
    },
	...
	createbet: function(book,favorite,backlay,odds,stake,id) {
		...
		books[book].bets.push({id:betnewid,favorite:favorite,stake:stake,backlay:backlay,odds:odds,timestamp:currenttimestamp});
